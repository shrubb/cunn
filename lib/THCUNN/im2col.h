...
template <typename Dtype, typename Acctype>
void col2im(
      cudaStream_t stream, const Dtype* data_col, const int channels,
      const int height, const int width,
...
// Same as col2im, but again with different data layout:
// * `data_col` is `batch_size` tensors of size 
// (kW*kH) x (outputHeight*outputWidth) concatenated over the SECOND dimension.
// * `data_im` is actually `gradInput` of size
// (batch_size) x (nInputPlane) x (outputWidth*outputHeight).
// * The results are put non-adjacently, namely into `gradInput[0][inPlaneIdx]`, 
// `gradInput[1][inPlaneIdx]`, ..., `gradInput[batch_size-1][inPlaneIdx]`.
template <typename Dtype, typename Acctype>
__global__ void col2im_depthwise_kernel(
      const int n, const Dtype* data_col,
      const int height, const int width,
      const int batch_size, const int nInputPlane, const int inPlaneIdx,
      const int kernel_h, const int kernel_w,
      const int pad_h, const int pad_w,
      const int stride_h, const int stride_w,
      const int dilation_h, const int dilation_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    data_im[(c_im * nInputPlane + inPlaneIdx) * height * width + index % (height * width)] = 
      ScalarConvert<Acctype, Dtype>::to(val);
  }
}

template <typename Dtype, typename Acctype>
void col2im_depthwise(
      cudaStream_t stream, const Dtype* data_col,
      const int batch_size, const int nInputPlane /* per one sample */,
      const int inPlaneIdx, const int height, const int width,
      const int patch_h, const int patch_w, const int pad_h,
      const int pad_w, const int stride_h, const int stride_w,
      const int dilation_h, const int dilation_w, Dtype* data_im) {
  int height_col = (height + 2 * pad_h - (dilation_h * (patch_h - 1) + 1))
  ...
  // To avoid involving atomic operations, we will launch one kernel per
  // bottom dimension, and then in the kernel add up the top dimensions.
  col2im_depthwise_kernel<Dtype, Acctype> <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>> (
      num_kernels, data_col, height, width, 
      batch_size, nInputPlane, inPlaneIdx,
      patch_h, patch_w, pad_h, pad_w, stride_h, stride_w,
      dilation_h, dilation_w,
      height_col, width_col, data_im
  );
